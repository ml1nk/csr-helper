...
let csv1 = fs.readFileSync(path.join(__dirname,'bulk','bulk-template-doubleid.csv')).toString('binary');
...
let csv2 = fs.readFileSync(path.join(__dirname,'bulk','bulk-template-shortpassword.csv')).toString('binary');
console.log("<bulk> = api.import.bulk(<csv content>,\"OU1\",\"OU2\"):");
try {
...
let csv3 = fs.readFileSync(path.join(__dirname,'bulk','bulk-template-wrongcount.csv')).toString('binary');
console.log("<bulk> = api.import.bulk(<csv content>,\"OU1\",\"OU2\"):");
try {
...
let csv4 = fs.readFileSync(path.join(__dirname,'bulk','bulk-template-empty.csv')).toString('binary');
console.log("<bulk> = api.import.bulk(<csv content>,\"OU1\",\"OU2\"):");
try {
...
let csv5 = fs.readFileSync(path.join(__dirname,'bulk','bulk-template.csv')).toString('binary');
console.log("<bulk> = api.import.bulk(<csv content>,\"OU1\",\"OU2\"):");
let bulk = api.import.bulk(csv5,"OU1","OU2");
console.log(bulk);
console.log("await api.export.bulk(<bulk>,\"nodebuffer\") > bulk.zip");
let bulk_zip = api.export.bulk(bulk,"nodebuffer");
bulk_zip.then((zip)=>{
